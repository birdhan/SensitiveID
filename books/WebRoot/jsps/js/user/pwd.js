...
function validateNewloginpass() {
	var bool = true;
	$("#newpassloginError").css("display", "none");
	var value = $("#newpass").val();
	if (!value) {// 非空校验
		$("#newpassloginError").css("display", "");
		$("#newpassloginError").text("新密码不能为空！");
		bool = false;
	} else if (value.length < 3 || value.length > 20) {// 长度校验
		$("#newpassloginError").css("display", "");
		$("#newpassloginError").text("新密码长度必须在3 ~ 20之间！");
		bool = false;
	}
	return bool;
}
