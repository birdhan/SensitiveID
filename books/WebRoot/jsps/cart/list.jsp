	...
	$(".jian").click(function() {
		...
		if(quantity == 1) {
			if(confirm("您是否真要删除该条目？")) {
				location = "/books/CartItemServlet?method=batchDelete&cartItemIds=" + id;
			}
		} else {
...
function batchDelete() {
	...
	location = "/books/CartItemServlet?method=batchDelete&cartItemIds=" + cartItemIdArray;
}
...
<table width="95%" align="center" cellpadding="0" cellspacing="0">
   ...
   <c:forEach items="${cilist }" var="ci">
	<tr align="center">
		<td align="left">
			<input value="${ci.cartItemId }" type="checkbox" name="checkboxBtn" checked="checked"/>
		</td>
		<td align="left" width="70px">
			<a class="linkImage" href="<c:url value='BookServlet?method=findLoad&bid=${ci.book.bid }'/>"><img border="0" width="54" align="top" src="<c:url value='/${ci.book.image_b }'/>"/></a>
		</td>
		<td align="left" width="400px">
		    <a href="<c:url value='BookServlet?method=findLoad&bid=${ci.book.bid }'/>"><span>${ci.book.bname }</span></a>
		</td>
		<td><span>&yen;<span class="currPrice">${ci.book.currPrice }</span></span></td>
		<td>
			<a class="jian" id="${ci.cartItemId }Jian"></a><input type="text" class="quantity" readonly="readonly" id="${ci.cartItemId }Quantity"  value="${ci.quantity }"/><a class="jia" id="${ci.cartItemId }Jia"></a>
		</td>
		<td width="100px">
			<span class="price_n">&yen;<span class="subTotal" id="${ci.cartItemId }Subtotal">${ci.total }</span></span>
		</td>
		<td>
			<a href="<c:url value='CartItemServlet?method=batchDelete&cartItemIds=${ci.cartItemId}'/>">删除</a>
		</td>
	</tr>
</c:forEach>
	...
	<form id="jieSuanForm" action="<c:url value='CartItemServlet'/>" method="post">
		...
		<input type="hidden" name="method" value="loadCI"/>
	</form>
